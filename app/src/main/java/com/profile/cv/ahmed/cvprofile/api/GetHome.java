...
public class GetHome extends AsyncTask<String, Void, HomeModelMain> {

	private String URL =  "http://cv-campany.com/API/ar/home/last";
	private ProgressDialog dialog;
	private OnLoadingComplete callback;
	private Context context;

	public GetHome(Context context, OnLoadingComplete cb) {
	...
	@Override
	protected HomeModelMain doInBackground(String... params) {
		...
		if (responseJSON != null && responseJSON.length() > 1) {
			...
			gson = gb.create();
			try {
				obj = gson.fromJson(responseJSON, HomeModelMain.class);
	...
	@Override
	protected void onPostExecute(HomeModelMain result) {
		if (dialog.isShowing()) {
			dialog.dismiss();
		}
		if (result != null) {
